...
## 9. 当前 Playwright 用例落地清单（代码事实）

### 9.1 创建页主线用例（`apps/tests/test/playwright/dashboard/*`）
- `quest-create.bootstrap.spec.ts`
  - `QC-M0-001` 新建页加载默认 Step1。
  - `QC-M0-002` `step=2/3` 路由进入指定步骤。
  - `QC-M0-003` `id + isFastCreate` 自动跳转 quick-create（依赖 `PW_CREATE_CAMPAIGN_FAST_CREATE_ID`，缺失则 `skip`）。
  - `QC-M1-001` 底部左右箭头切换步骤。
  - `QC-M1-002` Header `Next Step` 连续推进并在 Step3 显示 `Release`。
  - `QC-M1-003` Step 未完成时 `Release` disabled。
- `quest-create.info.spec.ts`
  - `QC-M2-001` Title 缺失导致无法 Release。
  - `QC-M2-003` `No End Time` 开关可见且可切换。
  - `QC-M2-004` Permission 默认 Private，可切 Public。
  - `QC-M20-001` Title 长度限制 80。
  - `QC-M20-003` Token 类型下 `No End Time` 不展示。
  - `QC-M2-002/003` 作为辅助流，完成 Step1 后可进入 Step3（不做 Step3 发布态强断言）。
- `quest-create.reward.spec.ts`
  - `QC-M3A-001` 默认 Points。
  - `QC-M3A-002` OAT/NFT 类型切换渲染。
  - `QC-M12-001` Token 奖励下默认 FCFS（通过 raffle 说明文案等 UI 联动校验）。
  - `QC-M13-001/002` OAT/NFT 下 Gas 配置可见。
  - `QC-M13-004` Custom Reward 不展示 Gas 配置。
  - `QC-M15-001/002` Token 子类型 Fixed/Luck-Based 切换。
- `quest-create.task-setting.spec.ts`
  - `QC-M4-001` 默认 Task Group/SetUpCred 入口可见。
  - `QC-M6-001` Participate Requirement 开关可切换。
  - `QC-M7-001` Referral 关闭时配置体不展示。
  - `QC-M8-001` Collect Info 开关可切换。
  - `QC-M9-001` User Agreement 默认关闭且输入框不展示。
- `quest-create.task-setting.credentials.spec.ts`
  - `QC-M5-001` 打开 credential 选择弹窗。
  - `QC-M5-002` 添加 Visit a Page 后 EditingCredentials 可见。
  - `QC-M16-001` credential 弹窗搜索输入可用。
  - `QC-M16-006` Recurring 第 2 个 credential 限制当前为占位 `test.skip(true)`。
- `quest-create.task-setting.advanced.spec.ts`
  - `QC-M17-001/002` 多维 credential 与 Multi-Entry Rule 配置链路（通过公共 helper 执行）。
  - `QC-M18-003` Boost 场景下 `Verify Before Tasks` 与 `advanced-sybil-prevention-switch` 可见。
- `quest-create.release.spec.ts`
  - `QC-M10-001` Save Draft 成功并带 `id`。
  - `QC-M10-002` Preview 无 id 禁用，有 id 可用。
  - `QC-M10-003` Step3 点击 Release 出现确认窗（`Release Now`）。
  - `QC-M10-004` 纯社媒类任务触发 Reminder（`Ignore and Release`）。
  - `QC-M11-001` active 编辑态回填（依赖 `PW_CREATE_CAMPAIGN_ACTIVE_ID`，缺失则 `skip`）。
  - `QC-M11-004` draft 编辑态可再次保存（依赖 `PW_CREATE_CAMPAIGN_DRAFT_ID`，缺失则 `skip`）。

### 9.2 历史/调试型创建用例
- `quest-creat-custom-visit.spec.ts`
  - 场景：Custom Reward + Visit a Page 的三步创建流程冒烟。
  - 特点：无 `QC-*` 编号、以截图记录为主，定位为调试/回归辅助脚本。

### 9.3 跨端端到端用例（`apps/tests/test/playwright/core/*`）
- `quest-e2e-points-follow-space-participant.spec.ts`
  - 覆盖“创建（Points + Follow Space）-> Release -> App 端验证”的完整闭环。
  - 当前仅 Follow Space 自动化验证启用，Space Quest Participant 相关步骤已注释。
- `quest-e2e-custom-visit.spec.ts`
  - 覆盖“创建（Custom Reward + Visit a Page）-> Release -> App 端 verify”闭环。
  - 当前整体 `test.skip`，保留为全链路脚本模板。

### 9.4 现状差异与补充建议
- 规划文档覆盖面（M12~M21）显著大于当前自动化实装；当前实装以 P0 主链路 + 部分 P1 冒烟为主。
- `M14 Mystery Box`、`M21 OAT/NFT 高级配置`、`M16 分类 Tab 过滤`等仍主要停留在计划层，建议按批次 C/D 继续补齐。
- `QC-M17-001/002` 主要依赖 helper 执行，建议后续补充显式断言（保存成功 toast、规则回填值）增强可维护性。

### 9.5 运行依赖（环境变量）
- 基础：`PW_DASHBOARD_DOMAIN`、`PW_CREATE_SPACE_ID`。
- 编辑态/跳转：
  - `PW_CREATE_CAMPAIGN_DRAFT_ID`
  - `PW_CREATE_CAMPAIGN_ACTIVE_ID`
  - `PW_CREATE_CAMPAIGN_FAST_CREATE_ID`

## 10. 覆盖率表（计划 vs 当前自动化）

### 10.1 按 Playwright 文件覆盖

| spec 文件 | 关联模块 | 用例数 | 默认可执行 | 条件 skip/占位 | 覆盖状态 |
| --- | --- | ---: | ---: | ---: | --- |
| `quest-create.bootstrap.spec.ts` | M0/M1 | 6 | 5 | 1 | 已覆盖 |
| `quest-create.info.spec.ts` | M2/M20 | 6 | 6 | 0 | 已覆盖 |
| `quest-create.reward.spec.ts` | M3/M12/M13/M15 | 6 | 6 | 0 | 已覆盖 |
| `quest-create.task-setting.spec.ts` | M4~M9 | 5 | 5 | 0 | 已覆盖 |
| `quest-create.task-setting.credentials.spec.ts` | M5/M16 | 4 | 3 | 1 | 部分覆盖 |
| `quest-create.task-setting.advanced.spec.ts` | M17/M18/M19 | 2 | 2 | 0 | 部分覆盖 |
| `quest-create.release.spec.ts` | M10/M11 | 7 | 5 | 2 | 已覆盖 |
| `quest-creat-custom-visit.spec.ts` | 调试冒烟 | 1 | 1 | 0 | 辅助脚本 |
| `core/quest-e2e-*.spec.ts` | 跨端闭环 | 2 | 1 | 1 | 部分覆盖 |
| 汇总（创建主线 7 文件） | M0~M20 主路径 | 36 | 32 | 4 | 88.9% |

### 10.2 按模块族覆盖

| 模块族 | 计划范围 | 当前状态 | 备注 |
| --- | --- | --- | --- |
| 基础流程 | M0/M1/M2/M10/M11 | 已覆盖 | 创建、导航、保存、发布、编辑态主路径已落地 |
| 奖励主流程 | M3/M12/M13/M15 | 已覆盖 | 已有 Points/OAT/NFT/Token 与 Gas、分发、子类型冒烟 |
| 任务设置主流程 | M4~M9 | 已覆盖 | 开关与基础交互覆盖，深层校验未全量 |
| 凭证深度能力 | M16/M17 | 部分覆盖 | 搜索、Multi-Entry 有覆盖；Tab 过滤/Recurring 限制未全量 |
| 高级能力 | M18/M19/M20 | 部分覆盖 | Boost 与部分 Info 校验覆盖；高级防 Sybil 细分场景未全量 |
| 奖励深度配置 | M14/M21 | 未覆盖 | Mystery Box 与 OAT/NFT 高级配置仍在计划层 |

说明：
- 百分比按“创建主线 7 个 spec 文件中的自动化用例数”统计，不含历史调试脚本与跨端模板脚本。
- `条件 skip/占位` 包含依赖环境变量数据集的 `test.skip(...)` 以及显式占位 `test.skip(true)`。
