...
export async function clickParticipateButton(page: Page) {
  try {
    const claimButton = page
      .getByText('Participate', { exact: true })
      .locator('..')
...
export async function isCredentialCompleted(
  ...
  try {
    ...
    if (isCompleted) {
      ...
      await delay(3000) // 等待一段时间后再次检查，适用于需要等待外部操作完成的情况

      await page.screenshot({
        path: `test-results/${testId}-not-completed-${retryCount + 1}.png`,
        fullPage: true,
      })
      page.reload({ waitUntil: 'domcontentloaded' })
      return isCredentialCompleted(page, testId, retryCount + 1) // 递归调用继续检查
    }
...
export const excuteVisitPageQuest = async (
  ...
  // console.log('ready to click claim button')
  await clickParticipateButton(testPage)
