...
import fs8, { lstatSync, rmSync, readdirSync as readdirSync$1, statSync, unlinkSync, rmdirSync, renameSync, chmodSync } from 'fs';
...
async function unzipArchivePhantom(options) {
  const { archivePath, overwrite } = options;
  const archiveFileExtension = archivePath.split(".").slice(-1);
  const outputPath = archivePath.replace(`.${archiveFileExtension}`, "");
  const fileExists = fs2.existsSync(outputPath);
...
var EXTENSION_LOAD_TIMEOUT = 1e4;
var MAX_RETRIES = 3;
var RETRY_DELAY_BASE = 1e3;
var INITIAL_BROWSER_DELAY = 1e3;
var MAX_BLANK_PAGE_RETRIES = 5;
var RELOAD_DELAY = 1e3;
...
async function isPageBlank(page, extensionPath) {
  const app = (extensionPath2) => {
    if (extensionPath2.includes("metamask")) {
      return "METAMASK";
    } else {
      return "PHANTOM";
    }
  ...
  try {
    const appElementCount = await page.locator(APP_SELECTORS[app(extensionPath)]).count().catch(() => 0);
...
async function fixBlankPage(page, extensionPath) {
...
async function waitForExtensionOnLoadPage(context, extensionPath) {
  let retries = 0;
  console.log(
...
async function triggerCacheCreation(setupFunctions, hashes, downloadExtension, force) {
  ...
  return await Promise.all(
    Array.from(setupFunctions).map(async ([_, { fileName, setupFunction }], index) => {
      ...
      const cachePath = path2.join(cacheDirPath, funcHash || "unknown");
      const doesCacheDirExist = await fs2.exists(cachePath);
...
// src/createCache.ts
async function createCache(walletSetupDirPath, hashes, downloadExtension, force = false) {
...
var chmod = (path9, mode) => new Promise((res, rej) => fs8.chmod(path9, mode, (er, ...d) => er ? rej(er) : res(...d)));
var readdir = (path9) => new Promise((res, rej) => fs8.readdir(path9, { withFileTypes: true }, (er, data) => er ? rej(er) : res(data)));
var rename = (oldPath, newPath) => new Promise((res, rej) => fs8.rename(oldPath, newPath, (er, ...d) => er ? rej(er) : res(...d)));
var rm = (path9, options) => new Promise((res, rej) => fs8.rm(path9, options, (er, ...d) => er ? rej(er) : res(...d)));
var rmdir = (path9) => new Promise((res, rej) => fs8.rmdir(path9, (er, ...d) => er ? rej(er) : res(...d)));
var stat = (path9) => new Promise((res, rej) => fs8.stat(path9, (er, data) => er ? rej(er) : res(data)));
var lstat = (path9) => new Promise((res, rej) => fs8.lstat(path9, (er, data) => er ? rej(er) : res(data)));
var unlink = (path9) => new Promise((res, rej) => fs8.unlink(path9, (er, ...d) => er ? rej(er) : res(...d)));
var promises = {
  chmod,
  readdir,
  rename,
...
async function prepareExtensionPhantom() {
  const cacheDirPath = ensureCacheDirExists();
  const downloadResult = await downloadFile({
...
// src/utils/extractWalletSetupFunction.ts
...
var createGlobPattern = (walletSetupDir) => path2.join(walletSetupDir, "**", "*.setup.{ts,js,mjs}");
...
var isRunningInWsl = () => {
  try {
    const releaseContent = fs8.readFileSync("/proc/version", "utf8").toLowerCase();
...
`);
  const program = new Command().name(chalk.magenta("synpress")).description("A CLI for building the cache of wallet setup functions").argument("[dir]", "Directory containing the wallet setup functions", path2.join("test", WALLET_SETUP_DIR_NAME)).option(
    "--headless",
    "Build cache in the headless browser mode. Alternatively, set the `HEADLESS` env variable to `true`",
    false
  ).option("-f, --force", "Force the creation of cache even if it already exists", false).option("-d, --debug", "If this flag is present, the compilation files are not going to be deleted", false).option("-p, --phantom", "If this flag is present, Phantom extension will be installed instead of Metamask", false).helpOption(void 0, "Display help for command").addHelpText("afterAll", `
${footer}
`).parse(process.argv);
  let walletSetupDir = path2.join(process.cwd(), "test", WALLET_SETUP_DIR_NAME);
  if (program.args[0]) {
    walletSetupDir = path2.join(process.cwd(), program.args[0]);
  }
  ...
  console.log(chalk.greenBright("\u{1F680} Building the cache for wallet setup functions... \u{1F680}\n"));
...
export { CACHE_DIR_NAME, DEFAULT_METAMASK_VERSION, EXTENSION_DOWNLOAD_URL, PHANTOM_EXTENSION_DOWNLOAD_URL, WALLET_SETUP_DIR_NAME, cliEntrypoint, createCache, createTempContextDir, defineWalletSetup, downloadFile, ensureCacheDirExists, prepareExtension, prepareExtensionPhantom, removeTempContextDir, unzipArchive, unzipArchivePhantom };
