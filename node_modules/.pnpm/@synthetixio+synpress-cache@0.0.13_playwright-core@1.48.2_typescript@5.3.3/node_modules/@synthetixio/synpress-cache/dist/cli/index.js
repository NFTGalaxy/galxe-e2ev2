#!/usr/bin/env node
import fs, { lstatSync, rmSync, readdirSync as readdirSync$1, statSync, unlinkSync, rmdirSync, renameSync, chmodSync } from 'fs';
...
async function triggerCacheCreation(setupFunctions, hashes, downloadExtension, force) {
  ...
  return await Promise.all(
    Array.from(setupFunctions).map(async ([_, { fileName, setupFunction }], index) => {
      ...
      const cachePath = path5.join(cacheDirPath, funcHash || "unknown");
...
${footer}
`).parse(process.argv);
  let walletSetupDir = path5.join(process.cwd(), "test", WALLET_SETUP_DIR_NAME);
  if (program.args[0]) {
    walletSetupDir = path5.join(process.cwd(), program.args[0]);
  }
