...
import {
  type GasSettings,
  GasSettingValidation,
} from '../../../../type/GasSettings'
import { waitFor, waitForMetaMaskLoad } from '../../../utils/waitFor'
...
const attemptConfirmTransaction = async (
  ...
  if (gasSetting === 'site') {
    ...
    const confirmButton = notificationPage.locator(
      Selectors.ActionFooter.confirmActionButton,
    )
  ...
  const editGasFeeButton = notificationPage.locator(
    Selectors.TransactionPage.editGasFeeMenu.editGasFeeButton,
  )
  ...
  if (gasSetting === 'low') {
    await handleLowMediumOrAggressiveGasSetting(
      gasSetting,
      Selectors.TransactionPage.editGasFeeMenu.lowGasFee,
    )
  } else if (gasSetting === 'market') {
    await handleLowMediumOrAggressiveGasSetting(
      gasSetting,
      Selectors.TransactionPage.editGasFeeMenu.marketGasFee,
    )
  } else if (gasSetting === 'aggressive') {
    await handleLowMediumOrAggressiveGasSetting(
      gasSetting,
      Selectors.TransactionPage.editGasFeeMenu.aggressiveGasFee,
    )
  } else {
    await notificationPage
      .locator(Selectors.TransactionPage.editGasFeeMenu.advancedGasFeeButton)
      .click()

    await notificationPage
      .locator(
        Selectors.TransactionPage.editGasFeeMenu.advancedGasFeeMenu
          .maxBaseFeeInput,
      )
      .fill('')
    await notificationPage
    ...
    await notificationPage
      .locator(
        Selectors.TransactionPage.editGasFeeMenu.advancedGasFeeMenu.saveButton,
      )
      .click()
  }
...
const confirmTransactionAndWaitForMining = async (
  walletPage: Page,
  notificationPage: Page,
  options: GasSettings,
) => {
  await walletPage
    .locator(HomePageSelectors.activityTab.activityTabButton)
    .click()

  const waitForUnapprovedTxs = async () => {
    const unapprovedTxs = await walletPage
      .locator(HomePageSelectors.activityTab.pendingUnapprovedTransactions)
      .count()
  ...
  const waitForMining = async () => {
    ...
    const pendingTxs = await walletPage
      .locator(HomePageSelectors.activityTab.pendingApprovedTransactions)
      .count()
    const queuedTxs = await walletPage
      .locator(HomePageSelectors.activityTab.pendingQueuedTransactions)
      .count()
...
const rejectTransaction = async (notificationPage: Page) => {
  await notificationPage
    .locator(Selectors.ActionFooter.rejectActionButton)
    .click()
}
