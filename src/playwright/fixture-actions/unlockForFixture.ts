...
import {
  closeNewNetworkInfoPopover,
  closePopover,
  closeRecoveryPhraseReminder,
...
async function unlockWalletButReloadIfSpinnerDoesNotVanish(metamask: MetaMask) {
  try {
    ...
    if (e instanceof playwrightErrors.TimeoutError) {
      console.warn(
        '[UnlockWalletButReloadIfSpinnerDoesNotVanish] Unlocking MetaMask timed out. Reloading page...',
      )
...
async function retryIfMetaMaskCrashAfterUnlock(page: Page) {
  ...
  const isPopoverVisible = await page
    .locator(HomePage.selectors.popover.closeButton)
    .first()
    .isVisible()
  ...
  if (!isHomePageLogoVisible && !isPopoverVisible) {
    if (await page.locator(CrashPage.selectors.header).isVisible()) {
      const errors = await page
        .locator(CrashPage.selectors.errors)
        .allTextContents()

      console.warn(
        [
          '[RetryIfMetaMaskCrashAfterUnlock] MetaMask crashed due to:',
          ...errors,
        ].join('\n'),
      )
      ...
      try {
        ...
        // console.log(
        //   '[RetryIfMetaMaskCrashAfterUnlock] Successfully restored MetaMask!',
        // )
      } catch (e) {
        if (e instanceof playwrightErrors.TimeoutError) {
          throw new Error(
            [
              '[RetryIfMetaMaskCrashAfterUnlock] Reload did not help. Throwing with the crash cause:',
              ...errors,
            ].join('\n'),
          )
        }
