...
export const waitForMetaMaskWindowToBeStable = async (page: Page) => {
  ...
  if (
    (await page.locator(ErrorSelectors.loadingOverlayErrorButtons).count()) > 0
  ) {
    const retryButton = await page.locator(
      ErrorSelectors.loadingOverlayErrorButtonsRetryButton,
    )
    await retryButton.click()
    await waitForSelector(
      LoadingSelectors.loadingOverlay,
      page,
      DEFAULT_TIMEOUT,
    )
  }
...
export const fixCriticalError = async (page: Page, maxRetries = 5) => {
  for (let times = 0; times < maxRetries; times++) {
    if ((await page.locator(ErrorSelectors.criticalError).count()) > 0) {
      // console.log(
      //   `[fixCriticalError] Metamask crashed with critical error, refreshing.. (attempt ${times + 1})`,
      // )
      ...
      if (times <= 3) {
        ...
        const restartButton = await page.locator(
          ErrorSelectors.criticalErrorRestartButton,
        )
      ...
      // console.log(
      //   `[fixCriticalError] Metamask crashed with error, refreshing.. (attempt ${times + 1})`,
      // )
...
export async function waitFor(
  ...
  while (!reachedTimeout) {
    let nextTimeout = timeouts.at(
      Math.min(timeoutIndex++, timeouts.length - 1),
    ) as number
