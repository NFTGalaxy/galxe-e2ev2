...
// test/playwright/wallet-setup/basic.setup.ts
import { defineWalletSetup as defineWalletSetup2 } from "@synthetixio/synpress-cache";
...
// src/playwright/pages/HomePage/actions/closeSuiAndMonadScreen.ts
...
import { errors, expect as expect3 } from "@playwright/test";
...
async function importWalletFromPrivateKey(page, network, privateKey, walletName) {
  await expect4(page.locator(HomePage_default.accountMenu.accountButton)).toBeVisible();
  await page.locator(HomePage_default.accountMenu.accountButton).click();
  await expect4(page.locator(HomePage_default.accountMenu.addAccountMenu.addAccountButton)).toBeVisible();
  await page.locator(HomePage_default.accountMenu.addAccountMenu.addAccountButton).click();
  ...
  await expect4(page.locator('[data-testid*="fungible-token-row-"]').first()).toBeVisible({ timeout: 1e4 });
}
...
// src/playwright/utils/allTextContents.ts
import { z } from "zod";
...
async function renameAccount(page, currentAccountName, newAccountName) {
  ...
  await expect5(async () => {
    ...
    expect5(accountNames.length).toBeGreaterThan(0);
  }).toPass();
  ...
  await expect5(page.locator(HomePage_default.editAccountMenu.accountNameButton)).toContainText(newAccountName);
}
...
async function switchAccount(page, accountName) {
  ...
  await expect6(async () => {
    ...
    accountNames = await allTextContents(accountNamesLocators);
    expect6(accountNames.length).toBeGreaterThan(0);
  }).toPass();
...
// src/type/GasSettings.ts
import { z as z2 } from "zod";
var GasSettingValidation = z2.union([
  z2.literal("Slow"),
  z2.literal("Fast"),
  z2.literal("Average"),
  z2.object({
    maxBaseFee: z2.number(),
    priorityFee: z2.number(),
...
// src/prepareExtensionPhantom.ts
import { downloadFile, ensureCacheDirExists, unzipArchivePhantom } from "@synthetixio/synpress-cache";
import fs from "fs-extra";

// src/playwright/fixture-actions/unlockForFixture.ts
import { errors as playwrightErrors } from "@playwright/test";

// src/playwright/fixture-actions/getExtensionId.ts
import { z as z3 } from "zod";
var Extension = z3.object({
  id: z3.string(),
  name: z3.string()
});
var Extensions = z3.array(Extension);
...
var basic_setup_default = defineWalletSetup2(PASSWORD, async (context, walletPage) => {
